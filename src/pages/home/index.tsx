...
export const Home: React.FC = () => {
  ...
  const handleTodoCheck: (
    id: string
  ) => React.FormEventHandler<HTMLInputElement> = (id) => {
    return (e) => {
  ...
  const handleTodoDelete: (
    id: string
  ) => React.MouseEventHandler<HTMLButtonElement> = (id) => {
    return () => {
  ...
  const handleDragEnd = () => {
    ...
    const reorderedTodos = removeInsert(
      draggedIndex,
      droppedIndex,
      displayTodos
    );
    ...
    if (inputFilter === 'active')
      setTodos(mapToSorted(todos, reorderedTodos, (todo) => !todo.isCompleted));
    else if (inputFilter === 'completed')
      setTodos(mapToSorted(todos, reorderedTodos, (todo) => todo.isCompleted));
